...
function open2() {
    ...
    let wrapper = document.getElementById("wrapperOpen");
    let x=document.createElement("h1");
    x.textContent="שולה המוקשים "
    x.style.color="white"
    x.style.fontSize="15vh";
    x.style.padding="150px";
    x.style.fontFamily="Anka CLM";
    wrapper.style.width="100vw";
    wrapper.style.height="100vh";

    wrapper.style.backgroundSize="cover"
    wrapper.style.backgroundImage="url('../image/ship.jpg')";
        ...
        for (var i = 0; i < arrRows[level]; i++) {
            for (var j = 0; j < arrColumns[level]; j++) {
                if (list[myhelp].style.backgroundImage == '')
                    ...
                    if (list[myhelp].style.backgroundImage == 'url("../image/pf.png")')
                        mat2[myhelp] = 'f';
                    else
                        mat2[myhelp] = 1;
                }
...
function continueGame() {
    ...
    for (var i = 0; i < arrRows[level]; i++) {
        ...
        for (var j = 0; j < arrColumns[level]; j++) {
            ...
            x.setAttribute("data-column", j);
            if (arr[myCount] == 'f') {
                x.style.backgroundImage = "url(../image/pf.png)";
                countFlags--;
            }
            else
                if (arr[myCount] == 1) { 
                    x.style.backgroundImage = "url(../image/p" + helpMat[myCount] + ".JPG)"
                    x.style.opacity = '1';
                    countOpen++;
                }
            board.appendChild(x);
...
function rightClick() {
    ...
    if (event.target != event.currentTarget) {
        if (event.target.style.backgroundImage == "") {
            event.target.style.backgroundImage = "url(../image/pf.png)"
            countFlags--;
        }
        else
            if (event.target.style.backgroundImage == 'url("../image/pf.png")') { 
...
function open1(r, c) {
    ...
    if (x.style.backgroundImage != "")
        return;
    x.style.backgroundImage = "url(../image/p" + mat[r][c] + ".JPG)"
    x.style.opacity = '1';
    countOpen++;
    if (mat[r][c] == 0) {
        open1(r + 1, c)
    ...
    for (var i = 0; i < x.length; i++) {
        ...
        if (mat[r][c] == 'b') {
            ...
            if (level == 1)
                ...
                await wait(15);
            y.style.backgroundImage = "url(../image/fire.gif)";
        }    
    }
...
function openNav() {
    let x= document.getElementById("BoardMenu");
    x.style.borderBottom = "3.5px solid white";
    x.style.top = "0"
    let y=document.getElementById("openBoardMenu");
    y.style.top = "45.7vh";
    y.setAttribute("z-index", "5");
...
function dblclick() {
    ...
    for (let i = -1; i <= 1; i++) {
        for (let j = -1; j <= 1; j++) {
            ...
            if (z.style.backgroundImage == 'url("../image/pf.png")')
                value--;
